...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial.isEmpty) Stream.empty
      ...
      val nn = newNeighborsOnly(neighborsWithHistory(head._1, head._2), newExplored)
      head #:: from(initial.tail #::: nn, newExplored)
    }
  }
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.headOption.map(x => x._2.reverse).getOrElse(Nil)
